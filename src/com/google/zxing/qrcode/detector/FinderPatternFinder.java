...
label0:
        {
            ...
            int i1;
            for (i1 = 0; i >= i1 && j >= i1 && image.get(j - i1, i - i1); i1++)
            {
                ai[2] = ai[2] + 1;
            }
...
label1:
        {
            for (; i >= k1 && j >= k1 && !image.get(j - k1, i - k1) && ai[1] <= k; k1++)
            {
                ai[1] = ai[1] + 1;
            }
            ...
            for (; i >= k1 && j >= k1 && image.get(j - k1, i - k1) && ai[0] <= k; k1++)
            {
                ai[0] = ai[0] + 1;
            }
            ...
            i2 = image.getWidth();
            int j1;
            for (j1 = 1; i + j1 < l1 && j + j1 < i2 && image.get(j + j1, i + j1); j1++)
            {
                ai[2] = ai[2] + 1;
            }
        ...
        for (; i + k1 < l1 && j + k1 < i2 && !image.get(j + k1, i + k1) && ai[3] < k; k1++)
        {
            ai[3] = ai[3] + 1;
        }

        if (i + k1 >= l1 || j + k1 >= i2 || ai[3] >= k)
        {
            return false;
        }
        for (; i + k1 < l1 && j + k1 < i2 && image.get(j + k1, i + k1) && ai[4] < k; k1++)
        {
            ai[4] = ai[4] + 1;
        }
        ...
        return Math.abs((ai[0] + ai[1] + ai[2] + ai[3] + ai[4]) - l) < l * 2 && foundPatternCross(ai);
    }
...
_L6:
        ...
        throws NotFoundException
    {
        ...
        for (int l = i1 - 1; l < k1 && !flag;)
        {
            ...
            while (j < l1) 
            {
                ...
                if ((i & 1) == 0)
                {
                    if (i == 4)
                    {
                        ...
                        {
                            if (handlePossibleCenter(map, l, j, flag3))
                            {
                                byte byte0 = 2;
                                boolean flag1;
                                if (hasSkipped)
            ...
            if (foundPatternCross(map))
            {
                flag2 = flag;
                i = i1;
                if (handlePossibleCenter(map, l, l1, flag3))
                {
                    int k = map[0];
                    flag2 = flag;
                    i = k;
    ...
    protected final boolean handlePossibleCenter(int ai[], int i, int j, boolean flag)
    {
        ...
        if (!Float.isNaN(f))
        {
            ...
            if (!Float.isNaN(f1) && (!flag || crossCheckDiagonal((int)f, (int)f1, ai[2], k)))
            {
                float f2 = (float)k / 7F;
                k = 0;
                i = 0;
