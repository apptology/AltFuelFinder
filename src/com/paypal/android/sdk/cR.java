// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.paypal.android.sdk;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import android.widget.ArrayAdapter;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;
import android.widget.TextView;
import java.util.List;

// Referenced classes of package com.paypal.android.sdk:
//            bu, bt

public final class cR extends ArrayAdapter
{

    private int a;

    public cR(Context context, List list, int i)
    {
        super(context, 0, list);
        a = i;
    }

    public final void a(int i)
    {
        a = i;
    }

    public final View getView(int i, View view, ViewGroup viewgroup)
    {
        view = (String)getItem(i);
        LinearLayout linearlayout = new LinearLayout(viewgroup.getContext());
        RelativeLayout relativelayout = new RelativeLayout(viewgroup.getContext());
        linearlayout.addView(relativelayout);
        bu.a(relativelayout, null, "6dip", null, "6dip");
        Object obj = bu.a(viewgroup.getContext(), "", "checked");
        ((ImageView) (obj)).setId(2303);
        android.widget.RelativeLayout.LayoutParams layoutparams = bu.a(viewgroup.getContext(), "20dip", "20dip", 11);
        layoutparams.addRule(10);
        relativelayout.addView(((View) (obj)), layoutparams);
        ((ImageView) (obj)).setColorFilter(bt.b);
        bu.a(((View) (obj)), null, null, "8dip", null);
        if (i != a)
        {
            ((ImageView) (obj)).setVisibility(4);
        }
        obj = new TextView(viewgroup.getContext());
        ((TextView) (obj)).setId(2302);
        bu.b(((TextView) (obj)), 83);
        viewgroup = bu.a(viewgroup.getContext(), "20dip", "20dip", 9);
        viewgroup.addRule(0, 2303);
        relativelayout.addView(((View) (obj)), viewgroup);
        bu.b(((View) (obj)), "6dip", "6dip", "6dip", "6dip");
        ((TextView) (obj)).setText(view);
        return linearlayout;
    }
}
