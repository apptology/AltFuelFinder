...
public class MenuSectionsListFragment extends CommonListFragment
{

    public MenuSectionsListFragment()
    {
    }
...
_L3:
    ...
    private void openMenuItems(CommonListEntity commonlistentity)
    {
        if (commonlistentity != null && StringUtils.isNotEmpty(commonlistentity.getId()))
        {
            ...
            intent.putExtra("MENU_ITEM_ID", commonlistentity.getId());
    ...
    {
        items = (List)cacher().getData((new StringBuilder()).append("MENU_SECTIONS_PROPERTY").append(tabId).toString());
        return items != null;
    }
    ...
    {
        return String.format("menu_sections.php?app_code=%s&tab_id=%s", new Object[] {
    ...
    {
        items = JsonParser.parseMenuList(s);
        return cacher().saveData((new StringBuilder()).append("MENU_SECTIONS_PROPERTY").append(tabId).toString(), items);
    }
