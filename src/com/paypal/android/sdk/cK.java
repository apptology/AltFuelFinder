// Decompiled by Jad v1.5.8e. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.geocities.com/kpdus/jad.html
// Decompiler options: braces fieldsfirst space lnc 

package com.paypal.android.sdk;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import android.widget.ArrayAdapter;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;
import android.widget.TextView;
import java.util.ArrayList;
import java.util.Iterator;

// Referenced classes of package com.paypal.android.sdk:
//            bu, cJ, cG, cE, 
//            cM, cL, bt

public final class cK extends ArrayAdapter
{

    private int a;

    public cK(Context context, ArrayList arraylist, int i)
    {
        super(context, 0, arraylist);
        a = i;
    }

    private static void a(Context context, RelativeLayout relativelayout, cJ cj)
    {
        Object obj1 = new LinearLayout(context);
        ((LinearLayout) (obj1)).setId(2304);
        char c = '\u0900';
        ((LinearLayout) (obj1)).setOrientation(0);
        Object obj = bu.a(-2, -2, 1, 2301);
        ((android.widget.RelativeLayout.LayoutParams) (obj)).addRule(0, 2303);
        relativelayout.addView(((View) (obj1)), ((android.view.ViewGroup.LayoutParams) (obj)));
        obj = new TextView(context);
        ((TextView) (obj)).setId(2302);
        bu.b(((TextView) (obj)), 83);
        ((LinearLayout) (obj1)).addView(((View) (obj)));
        bu.b(((View) (obj)), "6dip", null, null, null);
        TextView textview = new TextView(context);
        bu.d(textview, 83);
        ((LinearLayout) (obj1)).addView(textview);
        bu.b(textview, "6dip", null, "6dip", null);
        if (cj.e())
        {
            obj1 = new TextView(context);
            ((TextView) (obj1)).setId(2306);
            c = '\u0902';
            bu.c(((TextView) (obj1)), 83);
            android.widget.RelativeLayout.LayoutParams layoutparams = bu.a(-2, -2, 1, 2301);
            layoutparams.addRule(0, 2303);
            layoutparams.addRule(3, 2304);
            relativelayout.addView(((View) (obj1)), layoutparams);
            bu.b(((View) (obj1)), "6dip", null, null, null);
            ((TextView) (obj1)).setText(cE.a(cG.av));
        }
        obj1 = cj.f().iterator();
        int j = 2400;
        int i = 2500;
        while (((Iterator) (obj1)).hasNext()) 
        {
            Object obj2 = (cL)((Iterator) (obj1)).next();
            TextView textview1 = new TextView(context);
            textview1.setId(j);
            textview1.setText((new StringBuilder()).append(((cL) (obj2)).a()).append(" ").append(((cL) (obj2)).b()).toString());
            android.widget.RelativeLayout.LayoutParams layoutparams1 = bu.a(-2, -2, 1, 2301);
            layoutparams1.addRule(0, i);
            if (textview1.getId() == 2400)
            {
                layoutparams1.addRule(3, c);
            } else
            {
                layoutparams1.addRule(3, textview1.getId() - 1);
            }
            relativelayout.addView(textview1, layoutparams1);
            bu.e(textview1, 83);
            bu.b(textview1, "6dip", null, null, null);
            textview1.setEllipsize(android.text.TextUtils.TruncateAt.END);
            textview1 = new TextView(context);
            textview1.setId(i);
            textview1.setText(((cL) (obj2)).c());
            obj2 = bu.a(-2, -2, 0, 2303);
            ((android.widget.RelativeLayout.LayoutParams) (obj2)).addRule(8, j);
            relativelayout.addView(textview1, ((android.view.ViewGroup.LayoutParams) (obj2)));
            bu.d(textview1, 85);
            bu.b(textview1, "6dip", null, "6dip", null);
            j++;
            i++;
        }
        ((TextView) (obj)).setText(cj.c());
        ((TextView) (obj)).setEllipsize(android.text.TextUtils.TruncateAt.END);
        textview.setText(cj.d());
        textview.setEllipsize(android.text.TextUtils.TruncateAt.END);
    }

    public final void a(int i)
    {
        a = i;
    }

    public final View getView(int i, View view, ViewGroup viewgroup)
    {
        view = (cJ)getItem(i);
        LinearLayout linearlayout = new LinearLayout(viewgroup.getContext());
        RelativeLayout relativelayout = new RelativeLayout(viewgroup.getContext());
        linearlayout.addView(relativelayout);
        bu.a(relativelayout, null, "6dip", null, "6dip");
        ImageView imageview = bu.a(viewgroup.getContext(), view.a(), "");
        imageview.setId(2301);
        android.widget.RelativeLayout.LayoutParams layoutparams = bu.a(viewgroup.getContext(), "30dip", "30dip", 9);
        layoutparams.addRule(10);
        relativelayout.addView(imageview, layoutparams);
        bu.a(imageview, "4dip", null, null, null);
        imageview = bu.a(viewgroup.getContext(), "", "checked");
        imageview.setId(2303);
        layoutparams = bu.a(viewgroup.getContext(), "20dip", "20dip", 11);
        layoutparams.addRule(10);
        relativelayout.addView(imageview, layoutparams);
        imageview.setColorFilter(bt.b);
        bu.a(imageview, null, null, "8dip", null);
        if (i != a)
        {
            imageview.setVisibility(4);
        }
        if (view.g())
        {
            Object obj = viewgroup.getContext();
            viewgroup = new TextView(((Context) (obj)));
            viewgroup.setId(2302);
            i = 2302;
            bu.b(viewgroup, 83);
            Object obj1 = bu.a(-2, -2, 1, 2301);
            ((android.widget.RelativeLayout.LayoutParams) (obj1)).addRule(0, 2303);
            relativelayout.addView(viewgroup, ((android.view.ViewGroup.LayoutParams) (obj1)));
            bu.b(viewgroup, "6dip", null, null, null);
            if (view.e())
            {
                obj1 = new TextView(((Context) (obj)));
                ((TextView) (obj1)).setId(2306);
                i = 2306;
                bu.c(((TextView) (obj1)), 83);
                android.widget.RelativeLayout.LayoutParams layoutparams1 = bu.a(-2, -2, 1, 2301);
                layoutparams1.addRule(0, 2303);
                layoutparams1.addRule(3, 2302);
                relativelayout.addView(((View) (obj1)), layoutparams1);
                bu.b(((View) (obj1)), "6dip", null, null, null);
                ((TextView) (obj1)).setText(cE.a(cG.av));
            }
            obj1 = new TextView(((Context) (obj)));
            ((TextView) (obj1)).setId(2305);
            bu.d(((TextView) (obj1)), 83);
            android.widget.RelativeLayout.LayoutParams layoutparams2 = bu.a(-2, -2, 1, 2301);
            layoutparams2.addRule(3, i);
            relativelayout.addView(((View) (obj1)), layoutparams2);
            bu.b(((View) (obj1)), "6dip", null, null, null);
            obj = new TextView(((Context) (obj)));
            layoutparams2 = bu.a(-2, -2, 0, 2303);
            layoutparams2.addRule(8, 2305);
            relativelayout.addView(((View) (obj)), layoutparams2);
            bu.d(((TextView) (obj)), 85);
            bu.b(((View) (obj)), null, null, "6dip", null);
            viewgroup.setText(view.c());
            ((TextView) (obj1)).setText(view.d());
            ((TextView) (obj)).setText(view.f().a(0).c());
            return linearlayout;
        } else
        {
            a(viewgroup.getContext(), relativelayout, view);
            return linearlayout;
        }
    }
}
